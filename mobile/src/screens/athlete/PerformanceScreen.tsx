...
export default function PerformanceScreen() {
  ...
  return (
    ...
    >
      {grouped.length === 0 ? (
        <EmptyState title="Aucune performance enregistrée" subtitle="Log tes séries depuis l'écran d'une séance." />
      ) : (
            ...
            {exercises.map(({ exercise, series }) => (
              <View key={exercise} style={styles.exerciseBlock}>
                ...
                <View style={styles.seriesRow}>
                  {series.map((s) => (
                    <Text key={s.id} style={styles.seriesText}>
...
const styles = StyleSheet.create({
  ...
  exerciseName: { color: colors.text, fontWeight: '700', marginBottom: spacing.xs },
  seriesRow: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm },
  seriesText: { color: colors.textMuted, fontSize: fontSize.sm },
});
